...
- **The chain drafter / the SECOND LLM seam** — the narrative→chain front door (`backend/llm/chain_decomposition.py`). The names come from EDGAR-first **discovery** (below), not model recall; Sonnet only ORGANIZES that deterministic universe into segments + thesis-fit prose, into the authoring surface as `system_drafted`, to ratify. **Response-only** (writes nothing; promote is the only writer), **fail-open**, and it **never sources a number**. The loop: **draft → ratify → extract → score → promote**. See `docs/CHAIN_DRAFTER.md` (authoring) + `docs/DISCOVERY.md` (the names).
- **EDGAR-first discovery + the term set** — how the names are FOUND (`docs/DISCOVERY.md`). The thesis OWNS a persisted, tiered **term set** (`thesis.term_set`): **SIGNAL = operator SEEDS** (canonical compounds — a hit PLACES alone) vs **BROAD = keyword-gen proposals** (corroboration-only; the deterministic guard NEVER makes an LLM term SIGNAL), produced by **`POST .../terms`** (the operator owns the discriminating decision; a draft with no term set 503s). EFTS full-text enumerates the US-listed universe by **CIK** (deterministic, free, parallel under the shared rate limit) → **`classify`** into **PLACED** (≥1 signal) / **VERIFY** (broad-only, lower-confidence, **promotable**) → the **per-CIK reconciler** (`resolve_discovered_chain` — every in-master discovered CIK reaches the draft; completeness is the deterministic layer's, never the organizer's to lose) → **batched fail-open narration** (`narrate_placements` — thesis-fit prose for PLACED+VERIFY) + a **matched-term provenance tag** (`← psilocybin`). Governed by invariant #9 (recall is sacred). The LLM is demoted to keyword-gen + a directed **tail-sweep** (the foreign/ADR tail EFTS can't see).
- **Discovery net, made visible** — a discovered/proposed name is a *suggestion*; exact CIK/master membership DECIDES (`resolve_discovered_chain` by CIK; `_resolve_one` for off-universe names): a PLACED name auto-loads (drafted, prunable), a VERIFY name is shown lower-confidence and added by one click, an AMBIGUOUS one enters **only by an explicit operator pick** (ticker + CIK shown), an ABSENT one is shown-not-placed. The allowlist rule of invariant #2, on the UI.
