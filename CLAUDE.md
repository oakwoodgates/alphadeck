...
Excluding, skipping, or de-selecting a name grays it and keeps it on screen, reversible in one click —
...
- **Display signal** — a READ-ONLY computed indicator (`backend/signals/display/`): quiet per-name tape context (SMA position/flips, trailing returns, RVOL|8/|20, insider open-market buys — the Cockpit basket-table columns) shown beside the call, **never an input to it** — no `role`, cannot fire/arm/veto/grade, never persisted (compute-on-read: a day-varying field on the recorded CallCard would break the cron's `record_if_changed` idempotency). Its own registry + narrow protocol, structurally unable to import the call path. See `docs/DISPLAY_SIGNALS.md`.
...
- **Business type** (Business-Type M1 — replaced the retired size-tier "archetype"): the durable two-level MONITOR characterization of WHAT a name does — a `BusinessType` **leaf** (miner, utilities, biotech_pharma, …) + its `BusinessSupersector` rollup ("are the utilities moving?") + the **royalty/streaming overlay** (a company-NAME tell). Derived on read from the master's stored `sector` via the **operator-editable maps in `backend/securities/business_type/`** (sic_map.csv / supersectors.csv / royalty_patterns.txt / overrides.csv — edit those to re-rule the classification); the operator's per-security re-tag (`security_master.business_type`, store-on-diff) wins. Display identity, never a call input. The old `fund` marker is `instrument_kind='etf'`; `adjacent` (off-thesis) lives in the purity meter.
...
- **Authorship (honest by construction — Discovery cleanup S1)** — a field is the operator's **only if the operator changed it**. On a basket member, `authored_by` records who wrote the DESCRIPTION (`thesis_fit`): `system_drafted` ("model draft") until the operator **edits** the prose → `operator_edited` ("your words"); **nothing else flips it**. `operator_set` is **RETIRED for basket members** (it stays in the enum as the term set's load-bearing "seed" authorship): migration 0035 re-based stored rows, and promote **legacy-translates** a stray payload to `signed_off=true` + `system_drafted` — never stored. Promote otherwise **honors** authorship (never coerces); nothing drafted becomes a fact until the operator ratifies it.
- **Sign-off + the confidence ladder** (`BasketMember.signed_off`) — the per-NAME triage state on the discovery surface: **Excluded → Included (system-recommended) → Signed off** (excluded wins). Sign-off ENDORSES the company — a MARKER only: never authorship, never a promote gate (include gates; sign-off marks), never a call/score input (#4). A hand-add enters auto-signed-off; the flag is carried across a re-draft's re-roll (only an operator-EDITED description pins), and a signed-off name is never dropped (#9). A name the draft recommends into N links is **N `basket_member` rows** (same `security_id`, one per segment — real memberships); per-name link placement + conviction live on the TRIAGE / scored screen, not the discovery surface. The scored view is the **value-chain editor** — per-name link membership (#4, the DD-rail multi-select) + rename/reorder/add/remove links (#1–3), each an immediate-promote through the full-replace `POST /workbench/theses` via the pure `chainOps` transforms that MIRROR `useChainDraft` (never edit it); the first-screen editor (**✎ Edit the basket**) builds membership. A name in no real link floors to the visible **Discovered** pen, never `null`/gone (#9/WB#1). See `docs/TRIAGE.md`.
- **Recommendation** — a visible, **pending** LLM suggestion (a tier, a flag, a drafted placement/description) that **changes
  nothing until the operator confirms**; a drafted description becomes `operator_edited` (operator authority) only
  when the operator actually edits it — endorsing a name (sign-off) confirms the NAME, never the words. The
...
- **Three-gate TRIAGE + the cost thread** — TRIAGE runs **cheap cut** (zero API, on visible row data) → **mark for data** (the per-name / per-section opt-in; the control IS the trigger) → **finalize on data** (ratify per fact; the business-type re-tag lives on the rail). The **shortlist** (cheap-cut survivors) is the only set expensive operations touch: **cost is the operator's to spend, never ambient** — the third protective thread, peer to the trust invariants (decisions) and the interaction principles (workflow). See `docs/STAGE_MODEL.md` + `docs/TRIAGE.md`.
...
- **American English spelling, everywhere** — UI copy, comments, docstrings, docs: `favorable`, `color`, `behavior`, `gray`, `analyze`, `normalize`, `organize`, `recognize`, `labeled`, `canceled`. Nothing lints this and both spellings read as correct, so it is only ever caught by writing it right the first time — check any string a person reads (a `title=`, a `<th>`, a badge, a banner, an error). It has already reached the product once: "maximum favourable excursion" shipped in the Scoreboard's Timing hover. The comment/docstring backlog was swept in one dedicated change (2026-09-11, 218 instances, 166 of them `grey`) — a sweep like that stays its own change, never folded into a feature PR. Two things the sweep must never touch: a word that is an **identifier or a value crossing a boundary** (a wire/enum/JSON key, a DB column, a CSS class and its selector, `aria-labelledby`) and **quoted external content** (SEC filing fixtures, vendor field names) — those keep the source's spelling.
...
- **A recurring/daily fetch must stay FRESH — and for a heterogeneous cache, structurally, not with a flag.** Two mechanisms, don't conflate them. **(1) Prices** — `fetch_eod`/`fetch_csv` are cache-first (a cache *hit* returns STALE bars and never re-pulls — the #72 latent bug); the recurring path passes `force_refresh=True` (re-pull + overwrite), dev/`--no-live` stays cache-first, a cache MISS always fetches. **(2) EDGAR (and any heterogeneous cache)** — freshness is **key-classed / default-refresh**, NOT a per-call flag: immutable prefixes (`forms/*`) cache forever, every other prefix re-fetches on a 12h TTL when `allow_live`, so a new mutable endpoint is safe-by-default and no caller threads anything. R1 (#196) chose this over a per-call `force_refresh`/`ttl=` on EDGAR precisely because "the boolean wearing a timedelta — the next mutable endpoint forgets it" is how the ~11-day insider freeze happened. **The 12h TTL is the INTERACTIVE default, though: a RECURRING caller builds its client with the RECURRING TTL — five minutes (`RECURRING_CACHE_TTL_S`)** (`pipeline/daily.py`'s per-thesis client, `radar/spac.py`, `radar/shell_sweep.py`) — the per-CLIENT dial, because the 12h clock starts per COMPANY when that key was fetched, so any daytime read warmed the night's index and the nightly pass served it blind. **Five minutes, not zero, and the difference is MEASURED:** the three filing legs re-read one company's `submissions` key milliseconds apart and `_is_stale` is `now - mtime > ttl`, so zero costs 3 fetches where 1 is right (≈1,500 SEC requests a night on 500 names) while buying no freshness — and five minutes still closes the daytime gap (EDGAR accepts 06:00–22:00 ET, so nothing is filed in the five minutes before a 22:30 pass). `ingest_fundamentals` keeps 12h deliberately (quarterly series). See `docs/DATA_SOURCES.md` §cache-freshness + `docs/POSTMORTEM_CRON_FREEZE_2026-07.md`.
...
python -m pipeline.enrich_identity --live               # re-enrich master identity from submissions (bare = all baskets; --thesis/--universe)
