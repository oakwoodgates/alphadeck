...
- **Display signal** — a READ-ONLY computed indicator (`backend/signals/display/`): quiet per-name tape context (e.g. SMA position/flips) shown beside the call, **never an input to it** — no `role`, cannot fire/arm/veto/grade, never persisted (compute-on-read: a day-varying field on the recorded CallCard would break the cron's `record_if_changed` idempotency). Its own registry + narrow protocol, structurally unable to import the call path. See `docs/DISPLAY_SIGNALS.md`.
