...
class ScoreboardSummaryOut(BaseModel):
    ...
    # The nights the record has NOTHING honest for (``pipeline.backfill``; ``calls.reconstructed``):
    # as-ofs <= asof whose EVERY row is a reconstruction, ledger-wide, ascending. The record path
    # scores honest (nightly) rows only — a reconstructed row never opens or closes an episode — so a
    # night that also carries an honest row is scored from it and is NOT listed; the ones here are
    # named once and quietly, never as a per-row chip. Empty = every night has an honest row.
