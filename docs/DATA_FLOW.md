...
- **Read (serve a call):** `/theses/{id}/call?asof=` loads the thesis, **re-derives** the dated signals from the facts as-of that date (for a past `asof`, only facts RECORDED by the end of that market day — `serve_known_at`, #336; the basket is today's), runs the assembler, and returns the `CallCard` — and **writes nothing** (a GET is safe/idempotent; a refetch or as-of scrub accretes no rows). The call of record is logged separately — by the batch `pipeline.run` or the **daily cron** (`record_if_changed`, appended only on change) — and `calls` is **never read back to serve**. (The log *is* readable for accountability — `calls_repo.list_for_thesis` / `latest_for_thesis`, and the future Scoreboard.)
