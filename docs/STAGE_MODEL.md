...
| **SCORE** | Forward attribution — the platform's calls vs the operator's decisions (follow-blindly = v2). The record is now **instrumented**: per-episode record-provenance + a maturity horizon, an operator **ops surface** (freshness/health + Run-daily-now), a DB-snapshot safety net, and a per-episode **drill-down** (a price chart with a numbered insider/trigger/lifecycle overlay + an event ledger). *(v1 built — the Scoreboard.)* | System judges | `REPLAY.md` (the historical twin) → `SCOREBOARD.md` (the forward one) · `ADMIN.md` (the ops surface) |
