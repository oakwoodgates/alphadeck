...
| **Backtest hypothesis dials (H1 / H3 — the `insider_10b5_1_buy_weight` precedent: land the dial at today's value, measure on the lab, flip only on a pre-registered pass + operator sign-off).** All four are DORMANT: the defaults reproduce today byte-for-byte, pinned by `tests/calls/test_hypothesis_dials.py`. Adding them moved `config_hash` at their deploy even though no call changed — the fingerprint covers every declared field — but it caused **no re-record churn**: F1 stamps the hash on the `calls` row BESIDE the card, and `record_if_changed` compares only the card. | | | |
| `revenue_accel_grade` | `core` | **DORMANT (H1 leg 1)**: the call-strength class `signals/revenue_acceleration.py` emits. R6 fixed it CORE inline; the dial lifts that choice onto the config. Score follows the grade through `catalyst_conviction`'s existing pair (0.9 / 0.5) | the computed screen is the single largest source of arms — 465 of 969 armed member-nights vs the insider buy's 403 (dev copy, 2026-09-14 edge) — because a fundamentals screen is broad by nature (13–27% of each big basket). Demoting does NOT shorten the horizon (R8 decouples liveness from grade), so this and H5 stay separable |
| `revenue_accel_key` | `conviction` | **DORMANT (H1 leg 2)**: whether a revenue re-acceleration turns Key 1 or Key 2 | keyed on `SignalEvent.detector`, NOT on `Kind.CATALYST` — three detectors emit that kind (ratified catalyst, 8-K item, this screen), so moving the kind would move all three and the hypothesis could not be isolated. `detector` never reaches the wire, so no schema/enum/card change. Scope: the call composition only — M5b's theme-broadcast own-conviction check is a separate question and is untouched |
| `computed_conviction_requires_core_confirmation` | `False` | **DORMANT (H1 leg 3)**: when True, a member whose ONLY conviction is a computed screen arms solely against a volume-backed CORE confirmation; ratified / named-actor convictions unchanged | of the 395 arms carrying revenue re-acceleration without an insider buy, 213 (54%) were momentum-only flip entries. "Computed" is declared by PROVENANCE SOURCE in `signals/conviction_source.py` (`{"xbrl"}`), never by label prefix — the ratified catalyst's label is free operator text with no prefix, and labels get reworded |
| `breakdown_dearm_scope` | `all` | **DORMANT (H3)**: `all` (both de-arms) or `core_only` (the 200d structural de-arm only; the fast 8-day flip de-arm no-ops). OFF entirely stays `breakdown_dearm_enabled=False` — this dial never means off | gated in `detect_flip` beside the master switch, so a suppressed breakdown never enters the stream and leaves no counter-case or confidence haircut either. Motivating: 2026-08-18, breakdown risk fired on 58 of AI Memory's 90 names in one night and left 9 armed with both keys live |
| **Insider-SELL cluster risk (Band 03 S1 — LIVE since 2026-08-19; the sig-lab pass MEASURED the distribution and the priors below held through it — retune only via a fresh lab pass):** | | | |
| `insider_sell_enabled` | **True** | the master switch — LIVE by default | **flipped ON 2026-08-19** (sig-lab off-vs-on, prod: 58 fires / 5 theses / 0 withholds / 0 de-arms — capped `< risk_block_severity` so it can never withhold an arm; the corporate_risk precedent); the replay lab INHERITS this default (option A, 2026-08-20) — `--no-insider-sell` forces it OFF for an off-leg measure |
...
| **8-K item-code tape (Band 03 S3 — `corporate_risk` LIVE since 2026-08-18; the 2026-08-20 `1.01` decision demoted `1.01` out of the catalyst and added `3.02` as a live dilution risk; full doc `docs/CORPORATE_EVENTS.md`):** | | | |
| `corporate_catalyst_enabled` | False | the trigger-side master switch — detect() no-ops until ON | its OWN switch (it extends the LIVE catalyst family) — now **5.02-only** (1.01 demoted); the replay lab INHERITS this default (option A, 2026-08-20) — so a bare backtest correctly gets it OFF, and `--corporate-catalyst` forces it on for the lab |
| `corporate_risk_enabled` | **True** | the risk-side master switch — LIVE by default | **flipped ON 2026-08-17** (#280; pre-verified 0 withheld / 0 verdict changes on real prod data — the moderate 8-K counter-case); the replay lab INHERITS this default (option A, 2026-08-20) — `--no-corporate-risk` forces it OFF for an off-leg measure |
| `corporate_event_items["5.02"]` | flip / personnel / 0.5 / 90d | officer-director change → FLIP catalyst (the ONLY trigger item after the 1.01 demotion) | direction is ambiguous (a departure and a marquee hire file identically) — the evidence link does the work |
| `corporate_event_items["3.01"]` | risk / 0.50 / 180d | listing-deficiency notice — moderate, sub-veto | 180d ≈ a real exchange cure period |
| `corporate_event_items["3.02"]` | risk / 0.50 / 180d | unregistered equity sales (dilution) — moderate, sub-veto | **added 2026-08-20** (the `1.01` decision, option A): single-item (a 3.02 = dilution regardless of a co-located 1.01; MEASURED 99 live names, 32 with no 1.01); 0.50 < `risk_block_severity` 0.70 so it can never withhold an arm |
...
| **Share-count creep / ATM detection (Band 03 S4 — LIVE since 2026-08-19; the sig-lab pass MEASURED the distribution and the priors below held through it — retune only via a fresh lab pass):** | | | |
| `share_creep_enabled` | **True** | the master switch — LIVE by default | **flipped ON 2026-08-19** (sig-lab off-vs-on, prod: 61 fires / 5 theses / 0 withholds — real dilution, e.g. UEC +13.4% QoQ; `share_creep_score` 0.50 `< risk_block_severity` so it can never withhold an arm); the replay lab INHERITS this default (option A, 2026-08-20) — `--no-share-creep` forces it OFF for an off-leg measure |
...
| **SC 13D activist stake (Band 03 S5 — LIVE since 2026-08-20; the clean re-measure MEASURED the distribution and the priors below held through it — retune only via a fresh lab pass; full doc `docs/ACTIVIST_STAKE.md`):** | | | |
| `activist_stake_enabled` | **True** | the master switch — LIVE by default | **flipped ON 2026-08-20** (off-vs-on re-measure, prod: 29 clean warm fires / 10 arms / ≤4 per thesis — NO flood; every survivor a real 13D, filer ≠ subject & pct ≥ 5; the `_is_misattributed` screen removed the 5 self-filed/sub-5% mis-attributed fires — UEC, ISOU, CBRS, GME/EBAY — while keeping the valid cross-subject JAGU); a Key-1 CONVICTION that only WARMS (arming still needs a co-located confirmation), so it cannot arm a name alone; the replay lab INHERITS this default (option A, 2026-08-20) — `--no-activist-stake` forces it OFF for an off-leg measure |
...
| `catalyst_default_horizon_days` | 365 | liveness when no term is published | real term (period of performance) preferred. **MEASURED INERT (phase 1, 2026-09-18):** swept 90/180/365/540/730 over a year of tape and every point was byte-identical — pooled AND on its own `key1_source=ratified_catalyst` slice (46 episodes, the same −38.465% median at all five settings). It is a FALLBACK, read only when a fact publishes no `horizon_end`, and every catalyst fact on the tape publishes one. Not a broken dial — a dial with nothing to bind on |
| `catalyst_max_horizon_days` | `None` | **DORMANT (A2, phase 1b)**: a CAP on a PUBLISHED agreement term. `None` = no cap = today, byte-identically (the `insider_10b5_1_buy_weight` precedent: land it at today's value, measure it on the lab, flip only on a pre-registered pass + operator sign-off) | the lever the row above turned out not to be. A single 2026 DOE OTA (OKLO, DENE0009589, term to 2029-07-01) buys **1,238 days** of signal validity off one event; whether an edge survives that long is the question. **The asymmetry is deliberate** — the cap binds only the published branch, so at a 90-day cap a five-year agreement is live 90 days while a term-less catalyst still gets the 365-day default: "we do not believe the edge outlives N days" and "we do not know this agreement's term" are different statements and have a dial each. A DETECTOR dial (it changes what FIRES — a catalyst can age out under it — so the backtest's cached event stream is keyed on it). Adding it moved `config_hash` (`f6bfc4cd` → `83f0bf5f`) with no call change and, as with the HD dials, **no re-record churn**: the hash rides the `calls` row beside the card and `record_if_changed` compares only the card |
