...
| `insider_sell_enabled` | **True** | the master switch — LIVE by default | **flipped ON 2026-08-19** (sig-lab off-vs-on, prod: 58 fires / 5 theses / 0 withholds / 0 de-arms — capped `< risk_block_severity` so it can never withhold an arm; the corporate_risk precedent); replay's `--insider-sell` still forces it on for the backtest |
...
| `corporate_catalyst_enabled` | False | the trigger-side master switch — detect() no-ops until ON | its OWN switch (it extends the LIVE catalyst family) — now **5.02-only** (1.01 demoted); replay's `--corporate-catalyst` forces it on for the lab |
| `corporate_risk_enabled` | **True** | the risk-side master switch — LIVE by default | **flipped ON 2026-08-18** (#280; pre-verified 0 withheld / 0 verdict changes on real prod data — the moderate 8-K counter-case); replay's `--corporate-risk` still forces it on for the backtest |
...
| `share_creep_enabled` | **True** | the master switch — LIVE by default | **flipped ON 2026-08-19** (sig-lab off-vs-on, prod: 61 fires / 5 theses / 0 withholds — real dilution, e.g. UEC +13.4% QoQ; `share_creep_score` 0.50 `< risk_block_severity` so it can never withhold an arm); replay's `--share-creep` still forces it on for the backtest |
